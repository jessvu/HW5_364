...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/hw5"
## Provided:
...
class UpdateInfoForm(FlaskForm):
    updateItem = StringField("Update the priority of your item on the list.", validators=[Required()])
    submit = SubmitField('Update')

# TODO 364: Define a DeleteButtonForm class for use to delete todo items
class DeleteButtonForm(FlaskForm):
    deleteItem = StringField("Enter the name of the list you want to delete.", validators=[Required()])
...
def one_list(ident):
    ...
    items = lst.items.all()
    for item in items:
        list_items.append((item.description, item.priority))
...
def update(item):
    ...
    if form.validate_on_submit():
        priority_update = form.updateItem.data
